"""Sub-agent system — built-in agent types plus custom fork-return agents.
Mirrors Claude Code AgentTool: explore (read-only), plan (structured), general (full tools),
and supports user-defined agents via .miniagent/agents/*.md."""
...
# explore/plan sub-agents get read-only tools only to avoid modifying project files or system state.
READ_ONLY_TOOLS = {"read_file", "list_files", "grep_search"}

# explore sub-agent system prompt: read-only codebase search specialist focused on fast, clear reporting.
...
# plan sub-agent system prompt: read-only analysis that returns a structured implementation plan.
...
# general sub-agent system prompt: full toolset except agent, for independent complex tasks.
...
# In-process cache for custom agent discovery to avoid rescanning .miniagent/agents on every agent tool call.
_cached_custom_agents: dict[str, dict] | None = None


def _discover_custom_agents() -> dict[str, dict]:
    """Discover user- and project-level custom agents keyed by agent name."""
    ...
    _load_agents_from_dir(Path.home() / ".miniagent" / "agents", agents)
    # Project-level (higher priority, overwrites)
    _load_agents_from_dir(Path.cwd() / ".miniagent" / "agents", agents)
...
def _load_agents_from_dir(directory: Path, agents: dict[str, dict]) -> None:
    """Load Markdown agent definitions from a directory into the agents dict."""
    ...
    for entry in directory.iterdir():
        ...
        try:
            ...
            if "allowed-tools" in meta:
                # allowed-tools is a comma-separated whitelist; default grants all non-agent tools.
            ...
            # A single bad custom agent file must not break startup or other agents.
            pass
...
def get_sub_agent_config(agent_type: str) -> dict:
    """Build runtime system_prompt and tools config for an agent type."""
    custom = _discover_custom_agents().get(agent_type)
    if custom:
        if custom["allowed_tools"]:
            # Custom agents with an explicit whitelist receive only those tools.
            tools = [t for t in tool_definitions if t["name"] in custom["allowed_tools"]]
        else:
            # Sub-agents cannot call agent to avoid recursive sub-agent creation.
            tools = [t for t in tool_definitions if t["name"] != "agent"]
        return {"system_prompt": custom["system_prompt"], "tools": tools}

    # Built-in explore/plan agents share the same read-only tool set.
...
def get_available_agent_types() -> list[dict[str, str]]:
    """Return all agent type descriptions for the system prompt, built-in and custom."""
    types = [
...
def build_agent_descriptions() -> str:
    """Format custom agent types as Markdown for the main Agent system prompt."""
...
def reset_agent_cache() -> None:
    """Clear custom agent cache when refreshing .miniagent/agents during tests or runtime."""
    global _cached_custom_agents
