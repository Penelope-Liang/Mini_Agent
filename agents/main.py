...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description="MiniAgent — a minimal coding agent",
        add_help=False,
    )
...
def _is_anthropic_compatible_base_url(base_url: str | None) -> bool:
    ...
    return parsed.hostname == "api.anthropic.com" or path.endswith("/anthropic") or "/anthropic/" in path
...
async def run_repl(agent: Agent) -> None:
    ...
    while True:
        ...
        if inp == "/skills":
            ...
            if not skills:
                print_info("No skills found. Add skills to .miniagent/skills/<name>/SKILL.md")
            else:
...
def main() -> None:
    """CLI entry point: prepare runtime config, create Agent, and run one-shot or interactive mode."""
    # Parse CLI args such as --plan, --resume, --model, and optional one-shot prompt.
    args = parse_args()
    _load_env_file()

    if args.help:
        # Custom help text listing MiniAgent launch options and REPL commands.
        print("""
MiniAgent
...
Examples:
  ...
  MODEL=deepseek-chat APIKEY=sk-xxx API=https://api.deepseek.com/anthropic python -m agents.main "hello"
  MODEL=gpt-4o OPENAI_API_KEY=sk-xxx OPENAI_BASE_URL=https://your-host/v1 python -m agents.main "hello"
  python -m agents.main --resume
  python -m agents.main  # starts interactive REPL
""")
        sys.exit(0)

    # Map CLI boolean flags to the Agent permission mode.
    permission_mode = _resolve_permission_mode(args)
    # Model priority: CLI arg, then MODEL env var, then default.
    ...
    # Exit early with configuration guidance when no API key is available.
    ...
    # Create the main Agent; OpenAI-compatible and Anthropic backends use different base URL params.
    agent = Agent(
    ...
    # Resume session
    # --resume loads the latest session and restores history into the new Agent.
    if args.resume:
    ...
    # Join trailing CLI text into a one-shot prompt, otherwise start the interactive REPL.
    prompt = " ".join(args.prompt) if args.prompt else None

    if prompt:
        # One-shot mode
        # One-shot mode: exit after running the user prompt.
        try:
        ...
        # Interactive REPL
        # Interactive mode: loop on user input until exit.
