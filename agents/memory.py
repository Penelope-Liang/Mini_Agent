...
Core ideas:
1. Each project has its own memory directory keyed by a hash of the current working directory.
2. Each memory is a Markdown file with YAML frontmatter metadata.
3. MEMORY.md is an auto-generated index for quick display in the system prompt.
4. During chat, scan memory headers lightly, then use a side query to select relevant memories.
5. Retrieved memories are injected into the conversation as <system-reminder> blocks.
"""
...
# side query is an async callable: system prompt + user prompt -> model text.
# Typed as Any to avoid heavy Awaitable constraints at runtime.
SideQueryFn = Callable[[str, str], Any]  # actually Awaitable[str]


VALID_TYPES = {"user", "feedback", "project", "reference"}
MAX_INDEX_LINES = 200       # Max lines kept from MEMORY.md before system prompt injection.
MAX_INDEX_BYTES = 25000     # Max bytes kept from MEMORY.md before system prompt injection.


class MemoryEntry:
    """Full memory entry used by /memory listing and CRUD operations."""
...
def _project_hash() -> str:
    """Hash the current working directory so project memories stay isolated."""
    return hashlib.sha256(str(Path.cwd()).encode()).hexdigest()[:16]


def get_memory_dir() -> Path:
    """Return the current project memory directory, creating it if needed."""
    d = Path.home() / ".miniagent" / "projects" / _project_hash() / "memory"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _get_index_path() -> Path:
    """MEMORY.md is the index file for the current project memory files."""
...
def _slugify(text: str) -> str:
    """Convert a memory name into a short filename-safe slug."""
...
def list_memories() -> list[MemoryEntry]:
    """Read all memory files for the current project, newest first."""
    d = get_memory_dir()
    entries: list[MemoryEntry] = []
    for f in sorted(d.glob("*.md")):
        # MEMORY.md is an index, not a real memory entry.
        if f.name == "MEMORY.md":
            continue
        try:
            ...
            if not meta.get("name") or not meta.get("type"):
                continue
            # Invalid type falls back to project so bad files do not break listing.
            ...
            pass
    # Sort newest memories first for /memory display.
...
def save_memory(name: str, description: str, type: str, content: str) -> str:
    """Save a memory entry and refresh the MEMORY.md index."""
    d = get_memory_dir()
...
def delete_memory(filename: str) -> bool:
    """Delete a memory by filename and refresh the index."""
...
def load_memory_index() -> str:
    """Read MEMORY.md with length guards before system prompt injection."""
...
class MemoryHeader:
    """Lightweight memory header with metadata needed for recall filtering."""
...
MAX_MEMORY_FILES = 200                    # Max memory files considered during recall.
MAX_MEMORY_BYTES_PER_FILE = 4096          # Max bytes injected per memory file.
MAX_SESSION_MEMORY_BYTES = 60 * 1024      # Max total memory bytes injected per session.


def scan_memory_headers() -> list[MemoryHeader]:
    """Quickly scan memory headers without full bodies for low-cost recall filtering."""
    d = get_memory_dir()
    ...
    for f in d.glob("*.md"):
        ...
        try:
            stat = f.stat()
            raw = f.read_text()
            # Parse only the first 30 lines; frontmatter is usually complete there.
            first30 = "\n".join(raw.split("\n")[:30])
...
def format_memory_manifest(headers: list[MemoryHeader]) -> str:
    """Format memory headers into a manifest for the side query."""
    lines = []
    for h in headers:
...
def memory_age(mtime_ms: float) -> str:
    """Convert mtime into a human-friendly relative age string."""
...
def memory_freshness_warning(mtime_ms: float) -> str:
    """Warn that old memories may be stale and should be verified against current code."""
    days = max(0, int((time.time() * 1000 - mtime_ms) / 86_400_000))
    if days <= 1:
        return ""
...
class RelevantMemory:
    """A recalled memory ready for injection into the conversation."""
    ...
    @property
    def size(self) -> int:
        """Byte size of this memory content for session injection budgeting."""
...
async def select_relevant_memories(
    ...
    """
    Select memories most relevant to the current query.

    Flow:
    1. Scan memory headers without reading full bodies.
    2. Exclude memories already surfaced in this session.
    3. Ask the side query to choose up to 5 filenames from the manifest.
    4. Read selected memory bodies and truncate oversized files.
    5. Wrap results as RelevantMemory objects for injection.
    """

    # Scan all memory file headers.
    headers = scan_memory_headers()

    if not headers:
        return []

    # Exclude already surfaced memories.
    candidates = [h for h in headers if h.file_path not in already_surfaced]
    if not candidates:
        return []

    # Manifest is the candidate summary list for the side query.
    manifest = format_memory_manifest(candidates)

    # Call side_query to pick relevant memories by filename and description.
    try:
        ...
        # Side query may include prose; extract the JSON object only.
        match = re.search(r"\{[\s\S]*\}", text)
        if not match:
            return []

        # Parse JSON to get selected memory filenames.
        parsed = json.loads(match.group(0))
        selected_filenames = set(parsed.get("selected_memories", []))
        # Filter candidates by filename, up to 5 entries.
        selected = [h for h in candidates if h.filename in selected_filenames][:5]

        result: list[RelevantMemory] = []
        for h in selected:
            # Read each selected memory file.
            content = Path(h.file_path).read_text()
            # Truncate oversized files to avoid bloating context.
            if len(content.encode()) > MAX_MEMORY_BYTES_PER_FILE:
                content = content[:MAX_MEMORY_BYTES_PER_FILE] + "\n\n[... truncated, memory file too large ...]"

            # Build header text from mtime; stale memories include a freshness warning.
            ...
            # Return RelevantMemory objects to be formatted as <system-reminder> blocks.
        ...
        # Recall failures must not break chat; cancellation errors are silent.
...
class MemoryPrefetch:
    """Wrap the async memory recall task for polling by the Agent loop."""

    def __init__(self, task: asyncio.Task):
        self.task = task
        # consumed prevents reusing the same prefetch result twice.
        self.consumed = False

    @property
    def settled(self) -> bool:
        """Whether the prefetch task has completed."""
...
def start_memory_prefetch(
    ...
    """
    Start memory recall asynchronously before the main model reply.

    Returns a MemoryPrefetch handle, not memory content.
    The Agent loop polls completion and injects memories into the current message.
    """

    # Prefetch only for multi-word input to avoid side queries on short commands.
    if not re.search(r"\s", query.strip()):
        return None

    # Do not exceed the session memory byte budget.
    if session_memory_bytes >= MAX_SESSION_MEMORY_BYTES:
        return None

    # Require at least one real memory file in the directory.
    d = get_memory_dir()
    has_memories = any(f.suffix == ".md" and f.name != "MEMORY.md" for f in d.iterdir())
    if not has_memories:
        return None

    # Start async recall in parallel with the main model request.
...
def format_memories_for_injection(memories: list[RelevantMemory]) -> str:
    """Wrap recalled memories as system-reminder blocks for user-message injection."""
    parts = []
    for m in memories:
...
def build_memory_prompt_section() -> str:
    """
    Build the Memory System section injected into the system prompt.

    It tells the model:
    - where memory files live;
    - available memory types;
    - how to save memories via write_file;
    - what not to save;
    - what appears in the current MEMORY.md index.
    """
