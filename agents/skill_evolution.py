...
def get_evolution_dir() -> Path:
    return Path.cwd() / ".miniagent" / "skill-evolution"
...
def resolve_skill_file(skill_name: str, *, target: str = "active", active_dir: str = "") -> Path | None:
    ...
    if target in ("project", "active"):
        found = _find_skill_file_by_name(Path.cwd() / ".miniagent" / "skills", skill_name)
        if found:
            return found

    if target in ("user", "active"):
        found = _find_skill_file_by_name(Path.home() / ".miniagent" / "skills", skill_name)
        if found:
            return found
...
def _skills_root(target: str) -> Path:
    ...
    if target == "user":
        return Path.home() / ".miniagent" / "skills"
    return Path.cwd() / ".miniagent" / "skills"
...
def _maybe_prune_stale_skill(skill_name: str, stats: dict[str, Any]) -> bool:
    min_retrieved = _parse_int(os.environ.get("MINIAGENT_SKILL_USAGE_PRUNE_MIN_RETRIEVED"), 40)
    max_used = _parse_int(os.environ.get("MINIAGENT_SKILL_USAGE_PRUNE_MAX_USED"), 0)
    source = str(stats.get("source") or "").strip().lower()
    if source != "user" and os.environ.get("MINIAGENT_SKILL_PRUNE_PROJECT", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return False
