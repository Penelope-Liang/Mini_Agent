...
def get_active_tool_definitions(all_tools: list[ToolDef] | None = None) -> list[ToolDef]:
    """Filter and return active tool definitions, removing inactive deferred tools and metadata fields before API calls."""
...
tool_definitions: list[ToolDef] = [
    ...
    {
        "name": "skill",
        "description": "Invoke a registered skill by name. Skills are prompt templates loaded from .miniagent/skills/. Returns the skill's resolved prompt to follow.",
        "input_schema": {
            "type": "object",
            "properties": {
...
# Normalize Unicode quote characters to ASCII straight quotes.
def _normalize_quotes(s: str) -> str:
    s = re.sub("[\u2018\u2019\u2032]", "'", s)
    s = re.sub('[\u201c\u201d\u2033]', '"', s)
    return s

# Find the actual matched substring in file content
...
# Generate a simple unified diff-style output
...
def load_permission_rules() -> dict:
    ...
    user_settings = _load_settings(Path.home() / ".miniagent" / "settings.json")
    project_settings = _load_settings(Path.cwd() / ".miniagent" / "settings.json")
...
# ---------------- Execute tool calls -----------------------
# 'agent' and 'skill' are handled in agent.py to avoid circular imports."
...
async def execute_tool(
    ...
    # Search for and activate deferred tools.
    if name == "tool_search":
